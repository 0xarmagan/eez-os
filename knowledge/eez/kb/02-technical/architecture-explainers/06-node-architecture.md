...
- **Multi-prover, not single-prover.** EEZ is described as proof-system agnostic and multi-prover-capable. Each rollup sets its own threshold (one or more) via its manager contract, and there is no protocol-enforced minimum of two. The single `prover` box in the diagram is explicitly flagged as a topology abstraction, not a single-prover claim.
