...
Why most rollups use two or more. A second proving system is a fallback against a bug or a compromise in the first. If one system is sound, the combination does not regress. Multi-prover is the security design intent, and a likely EEZ-zone policy recommendation, but it stays the rollup's configurable choice rather than a contract floor. You pick the systems and the threshold that fit your trust assumptions and your cost profile.
