...
It helps to read the seven in two groups. Properties 1, 2, and 3 are about the zone's posture towards you. It is open, fixed, and reversible. Properties 4, 5, and 6 are about your chain's relationship to its own design and to the other rollups. You keep your internals, you meet one firm requirement, and you inherit a strong cross-chain security guarantee. Property 7 is about what the zone does not entangle you with on L1. Each property is a genuine list item, so it gets its own numbered section, but the reasoning for each is in prose.
...
## 4. Proof-system agnostic, with a minimum of two
...
There is one hard rule that builders must not miss. A rollup must use at least two proving systems. The minimum of two is enforced in the contract itself, not offered as a recommended default. The deck's proving structures (`ProofSystemBatchPerVerificationEntries`, `RollupIdWithProofSystems`, `_fetchVkMatrix`) build a per-rollup verification-key matrix, and `checkProofSystemsAndGetVkeys` reverts with `InvalidProofSystemConfig()` when the configuration is wrong. So "agnostic" means you choose which systems, for example Zisk plus SP1, or a ZK system plus a TEE. It does not mean you may run on one. A single prover is not a valid configuration.

Why two. A second proving system is a fallback against a bug or a compromise in the first. If one system is sound, the combination does not regress. For a builder this is a safety floor you inherit for free, and it is also a design lever, because you pick the two systems that fit your trust assumptions and your cost profile.
...
Concretely, sovereignty means the zone does not dictate your chain's internals. You set your own state transition function, which can be WASM, RISC-V, or something custom. You choose your accepted proving systems, subject to the minimum of two from property 4. You decide your own update mechanism, so how your chain evolves is your call, not the zone's. Native rollups can include ETH inside cross-rollup CALLs, with cross-rollup ETH transfers handled by rollup-level accounting maintained on L1. Rollup creation is itself permissionless.
...
Put the seven together and the offer is coherent. You join a fixed, permissionless contract that you can leave at will. You keep your state transition function, your update mechanism, and your choice of proving systems, with the single firm requirement of at least two. In exchange you get cross-rollup composability with a security guarantee modelled on Ethereum's own, and none of it ties you to a specific L1 pre-confirmation future.
...
- **Multi-prover is a requirement, not an option.** Property 4 means a rollup chooses which proving systems, but a minimum of two is enforced in the contract via `checkProofSystemsAndGetVkeys` / `InvalidProofSystemConfig()`. A single prover is not a valid configuration. Nothing here implies otherwise.
