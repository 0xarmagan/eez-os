...
The cleaner user route is account abstraction: bundle the whole sequence into a single transaction that uses transient storage instead of real storage. The recording describes this as much cheaper than the real-storage path (around 400K gas). A separate gas saving of roughly 10x is cited for submitting the L1-to-L2 call as a meta-transaction rather than a normal L1 call (~300K gas). In either case the user pays only if the transaction does not revert.
