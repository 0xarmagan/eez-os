...
*Explainer 5 of 8. Sources: `knowledge/eez/sources/dappcon-2026-eez-node-architecture.md` (DAPPCon 2026 EEZ Workshop, Jordi Baylina, 17 June 2026) and `knowledge/eez/sources/dappcon-2026-gnosis-chain-eez-talk.md` (Phillipe Schommers, the on-L1 seeding mechanism and orderflow routing). Treat as engineering-level founding material, not approved EEZ comms.*
...
Before going further, one status note. EEZ is not deployed yet. The composer described here is the first version on the roadmap. Nothing in this document means you can run a composer today. This is the design, not a live system.
...
There is also a cleaner route through account abstraction. Instead of a multi-transaction bundle, the whole sequence runs in one transaction using transient storage, which is roughly ten times cheaper than the full real-storage version. On this path the user pays only if the transaction does not revert.

This is also why cross-chain transactions cannot use the public L1 mempool. If a user sends a cross-chain L1 transaction to the public mempool, an ordinary Ethereum validator can pick it up and include it before the composer has seeded the state. The B-star call then reverts, because the answer was never written. This is not malicious front-running by the composer. It is an ordering requirement: the seeded state has to be in place before the call runs, so the orderflow has to reach the composer first, via the bundle path or account abstraction. EEZ does not name or define a "private mempool" as the solution. The pool that holds cross-chain orderflow is explicitly left undefined at this stage (covered below).
...
The deck's diagram shows the composer as a binary with several parts. Here is what each one does, as far as the diagram and deck describe it.
...
**Propose settlement.** The output step described above. The composer takes the last n blocks and creates the `postBatch` payload, sends it to the provers first, then to L1 through the L1 Builder. The final flow on the diagram is the composer submitting the bundle to the L1 Builder, which posts to L1. The bundle is up to three transactions in order: deploy the proxy if it does not exist yet, load the lookup table, then the user transaction that hits the proxy. The composer's proof transaction lands first; the user transaction lands second. Only the user transaction triggers the state change, so if there is no user transaction, nothing settles.
...
**There is no defined incentive mechanism for routing fees to the composer.** In this first version, EEZ does not define how a composer gets paid for its work. The team acknowledges an incentive is needed and names candidate mechanisms without committing to one: fees, higher fees, private orderflow, or L2 fees. That is a list of options, not a mechanism. Whether any of them produces reliable composer behaviour is an open question. A composer in binding mode commits to work, but cross-chain fee revenue is irregular, so a rational operator might default to the non-binding or optimistic path in quiet periods. Composer fee incentives are roadmap work, not a solved problem.
...
- **EEZ is not deployed.** The composer here is the first version on the roadmap. No one can run a composer today.
